...
macOS 専用の dotfiles です。Ghostty + zsh + tmux + Neovim + Codex CLI / Codex app を中心とした開発環境を管理します。Claude Code は legacy として残しています。
...
4. npm グローバルパッケージのインストール（legacy Claude Code CLI など）
5. 各設定ファイルをホームディレクトリへシンボリックリンク（Codex hooks を含む）
...
| `--link-only` | Homebrew・パッケージインストール・各種同期をスキップし、リンク作成、Ghostty テーマ確認、Codex hook 確認のみ実行。再セットアップや設定ファイルの追加時に使う |
...
`~/.codex/hooks.json` と `~/.codex/hooks/` にシンボリックリンクを貼ります。
設定ファイル更新時は Codex hook が `/export` と `bash bin/install.sh --link-only` の確認を促します。

### tmux

prefix キーは `Ctrl-z` です（`Ctrl-b` から変更）。
