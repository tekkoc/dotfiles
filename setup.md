...
| AI エージェント | Codex CLI / Codex app（Claude Code は legacy）[macOS] |
...
# 2. install.sh を実行（シンボリックリンク作成 + Codex hook 設定）
bash ~/dev/dotfiles/bin/install.sh

# 3. 設定ファイルだけを再反映する場合
bash ~/dev/dotfiles/bin/install.sh --link-only

# 4. Neovim 起動（lazy.nvim が自動インストール）
nvim
```
...
| `c`    | `codex`                                       |
| `cr`   | `codex resume`                                |
| `cl`   | `claude --dangerously-skip-permissions`（legacy） |
| `clr`  | `claude --resume --dangerously-skip-permissions`（legacy） |
...
ステータスバー: Night Owl カラーパレット（背景 #011627、アクセント #9FDAF4）。SSH セッション中は右端に `SSH` を赤字（`#e06c75`）で表示。

SSH インジケーターの仕組み: `.zshrc` が SSH セッション内で tmux 起動時に `tmux setenv -g SSH_CONNECTION` を実行してグローバル環境変数を設定し、ステータスバーが `tmux show-environment -g SSH_CONNECTION` で読み取る。
...
Homebrew PATH は `.zshenv` で設定（非ログインシェル・Codex / Claude Code 等でも有効）。
...
| `ssh`     | SSH 接続中に Ghostty の背景色をネイビー（`#0f1f35`）に変更し、切断後に catppuccin-mocha の背景色（`#1e1e2e`）へ復元。`BatchMode=yes` の非インタラクティブ接続はスキップ [macOS] |
