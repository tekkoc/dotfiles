...
> **凡例**
> - `[macOS]` — macOS 固有の設定。他の OS では代替手段が必要
> - マークなし — クロスプラットフォーム（macOS / Linux / WSL 共通）
...
- `[macOS]` Ghostty シェル統合を自動読み込み（`$GHOSTTY_RESOURCES_DIR` が設定されている場合）
...
| cmatrix | マトリックス風端末アニメーション |
| pipes.sh (`pipes`) | パイプスクリーンセーバー（ランダム描画） |
| colima `[macOS]` | macOS 向け軽量コンテナランタイム（Docker Desktop 代替）|
| docker / docker-compose | コンテナ操作 CLI |
| go | Go プログラミング言語 |
| php | PHP |
| tailscale `[macOS]` | VPN メッシュネットワーク（リモートアクセス） |
...
> `[macOS]` 個人アカウント（Auto Mode が使えないエンタープライズプランを除く）では `--enable-auto-mode` に置き換え可。

| エイリアス | 展開先 | 備考 |
|---|---|---|
| `c` | `claude --dangerously-skip-permissions` | `[macOS]` Auto Mode 非対応時のみこのフラグを使用 |
| `cr` | `claude --resume --dangerously-skip-permissions` | 直前のセッションを再開 |
...
| キー | 動作 |
|---|---|
| `<leader>uf` | ファイル検索（telescope） |
| `<leader>ub` | バッファ一覧（telescope） |
| `<leader>uh` | ヘルプ検索（telescope） |
| `<leader>ug` | テキスト検索（telescope live_grep） |
| `<leader>um` | 最近開いたファイル（telescope oldfiles） |
| `<leader>ff` | ファイラトグル（Neo-tree、カレントファイル追従） |
| `S` | 水平分割で開く（Neo-tree 内） |
| `V` | 垂直分割で開く（Neo-tree 内） |
| `<leader>o` | URL をブラウザで開く |
...
| telescope.nvim | ファジーファインダー（`<leader>uf/ub/uh/ug/um`）。`um` は oldfiles（最近開いたファイル） |
| telescope-fzf-native.nvim | telescope の fzf パフォーマンス強化 |
| nvim-treesitter | シンタックスハイライト（lua / rust / typescript / tsx / javascript） |
| lewis6991/gitsigns.nvim | Git 変更をサインカラムに表示 |
| nvim-neo-tree/neo-tree.nvim | ファイラ（サイドバー型、`<leader>ff` でトグル・カレントファイル追従） |
...
Brewfile で管理（`cask "tailscale"`）。両機を同じ Tailnet に参加させることで、外出先からも安全にアクセスできる。MagicDNS を有効にすると `<hostname>.<tailnet>.ts.net` で名前解決できる。

### SSH

`~/.ssh/config` はマシン固有のため dotfiles 管理外。接続先の設定は各マシンで手動管理する。

**SSH サーバ側（接続される側）の必要設定:**

- **システム設定 → 一般 → 共有 → 「リモートログイン」をオン**（SSH サーバが有効になる）
- 公開鍵を `~/.ssh/authorized_keys` に登録するとパスワード不要になる
...
Claude Code のタスク完了時またはユーザーの入力待ち状態になったとき、macOS の通知センター経由でデスクトップ通知を送る設定をしている。

---

## パッケージ管理

### Homebrew（`homebrew/Brewfile`）`[macOS]`
