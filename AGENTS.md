# AGENTS.md — dotfiles (macOS)

このファイルは Codex がこのリポジトリを扱うときに参照する指示書です。

---

## このリポジトリの目的

macOS 上での個人開発環境を Git で管理する dotfiles です。
主な構成要素は **Ghostty + zsh + tmux + Neovim + Codex CLI / Codex app** です。
Claude Code は legacy として残すが、新規記述は Codex を主にする。

---

## ファイル構成

```
dotfiles/
├── .claude/
│   └── commands/
│       ├── export.md       # /export スラッシュコマンド（dotfiles 実態から setup.md を更新）
│       ├── setup-import.md # /setup-import スラッシュコマンド（他環境の設定を取り込む）
│       ├── add-tool.md     # /add-tool スラッシュコマンド
│       ├── push.md         # /push スラッシュコマンド（main を origin/main へ push）
│       └── pull.md         # /pull スラッシュコマンド（origin/main を pull rebase で取り込み）
├── bin/install.sh          # セットアップスクリプト（冪等）
├── codex/
│   ├── hooks.json          # Codex hook 設定
├── claude/
│   ├── statusline.py       # Claude Code カスタムステータスライン（legacy）
│   └── hooks/              # Claude Code hooks（legacy）
├── ghostty/config          # Ghostty ターミナル設定
├── git/.gitconfig          # Git 設定
├── homebrew/Brewfile       # Homebrew パッケージ一覧
├── nvim/
│   ├── init.lua            # Neovim エントリポイント（lazy.nvim ブートストラップ）
│   └── lua/
│       ├── core.lua        # 基本設定（表示・編集・検索）
│       ├── keymap.lua      # キーマッピング（Leader=,）
│       ├── commands.lua    # カスタムコマンド（:Inbox, :Temp）
│       └── plugins.lua     # lazy.nvim プラグイン定義
├── setup.md                # 環境設定リファレンス（別環境再現用）
├── CHANGES.md              # 変更ログ
├── tmux/.tmux.conf         # tmux 設定
└── zsh/
    ├── .zshenv             # 全シェル共通の環境変数（最小限）
    ├── .zprofile           # ログインシェル用（Homebrew PATH 等）
    ├── .zshrc              # インタラクティブシェル設定
    └── .zsh/
        ├── aliases.zsh         # エイリアス
        ├── functions.zsh       # シェル関数
        ├── ohmyposh-theme.json # Oh My Posh カスタムテーマ
        ├── reference.md        # キーバインド・エイリアスのリファレンス（? で表示）
        └── completions/        # 補完スクリプト置き場
```

シンボリックリンク先（`install.sh` が作成）：

| リポジトリ内のパス             | リンク先                          |
|------------------------------|----------------------------------|
| `zsh/.zshenv`                | `~/.zshenv`                      |
| `zsh/.zprofile`              | `~/.zprofile`                    |
| `zsh/.zshrc`                 | `~/.zshrc`                       |
| `zsh/.zsh/`                  | `~/.zsh/`                        |
| `tmux/.tmux.conf`            | `~/.tmux.conf`                   |
| `ghostty/config`             | `~/.config/ghostty/config`       |
| `git/.gitconfig`             | `~/.gitconfig`                   |
| `codex/hooks.json`           | `~/.codex/hooks.json`            |
| `claude/statusline.py`       | `~/.claude/statusline.py`        |
| `nvim/`                      | `~/.config/nvim/`                |
| `lazygit/config.yml`         | `~/.config/lazygit/config.yml`   |

`install.sh` はシンボリックリンク作成に加え、`~/.codex/hooks.json` と通知 hooks を管理する。Claude Code の `~/.claude/settings.json` 設定は legacy として既存設定とマージする。

---

## 作業ルール

### 変更前に必ず確認すること

- **設定変更は小さく・1ファイルずつ**行う。複数ファイルを一括で変えない
- zsh の設定を変更したら `source ~/.zshrc` で反映確認を促す
- tmux の設定を変更したら `tmux source ~/.tmux.conf` で反映確認を促す
- `install.sh` を変更したら `--check` オプションで副作用を確認してから実行する

### 禁止事項

- `~/.zsh/local.zsh` への言及・編集（`.gitignore` 対象のプライベートファイル）
- `git/.gitconfig` への `user.name` / `user.email` の直接書き込み（`install.sh` が対話的に設定する）

### 提案の作法

- 設定変更を提案する際は**変更理由を1行で添える**
- 複数の選択肢がある場合は比較を示してから選択を委ねる
- 既存の設定との**競合・重複**がないか確認してから追記する

### 環境固有の設定の表記ルール

setup.md に設定を記述するとき、以下のバッジを使って環境依存を明示する。

| バッジ         | 意味                                                       |
|-------------|----------------------------------------------------------|
| `[macOS]`   | macOS 固有の機能・アプリに依存する設定（他 OS では動作しない）  |
| `[Mac mini]` | Mac mini（サーバー用途）でのみ必要な設定                     |

バッジなしの設定は、環境を問わず再現可能（クロスプラットフォーム / 汎用）とみなす。

**適用基準:**
- `[macOS]`: `pbcopy`、`Cmd+` キー、`macos-*` オプション、macOS 専用アプリ（Ghostty・Amethyst・Karabiner-Elements・Raycast など）
- `[Mac mini]`: サーバー運用に特有のもの（Tailscale による外部からのリモートアクセス、コンテナランタイムなど）

**表記方法**: 表のセル末尾・箇条書き末尾・セクション見出しに括弧なしで付ける。

```markdown
| tailscale-app | VPN メッシュネットワーク [Mac mini] |
| Amethyst      | タイル型ウィンドウマネージャ [macOS] |

- `macos-option-as-alt = left` — 左 Alt を Meta キーとして使用 [macOS]
```

---

## よくある作業パターン

### 新しい CLI ツールを導入する

1. `homebrew/Brewfile` に `brew "ツール名"` を追加
2. `zsh/.zsh/aliases.zsh` に必要なエイリアスを追加
3. `zsh/.zshrc` に初期化コード（`eval "$(tool init zsh)"` 等）を追加
4. 必要なら `zsh/.zsh/completions/` に補完スクリプトを追加

### tmux の設定を変更する

`tmux/.tmux.conf` を編集する。変更後のコマンドは：

```bash
tmux source ~/.tmux.conf
```

キーバインドを追加するときは既存の prefix との衝突を確認すること（現在の prefix は `Ctrl-z`）。

### Ghostty の設定を変更する

`ghostty/config` を編集する。変更は Ghostty の再起動か `Cmd+,` で設定を開いて確認。

日本語まわりで注意が必要な既知設定：
- `font-feature = -liga -dlig -calt` — CJK 入力時のリガチャ誤動作を回避するため無効化済み。削除しない
- `macos-option-as-alt = left` — zsh の Alt キーバインドを有効にするため設定済み。削除しない

### setup.md を更新する

`setup.md` は**別環境での再現に必要な情報**をまとめたドキュメント。設定ファイルの有無にかかわらず、「この環境はこう動作する」という事実が変わったときに更新する。

**必ず更新する変更（スキップ禁止）:**
- ツール・プラグインの追加・削除
- キーバインド・エイリアス・関数の変更
- 設定値の変更（フォントサイズ、テーマ、オプションなど）
- OS の機能や外部サービスとの連携（通知、IME 設定など）
- プロンプト・テーマの見た目の変更
- `Brewfile` / `npm/packages.txt` のパッケージ追加・削除

**更新方法**: `/export` スラッシュコマンドを実行する。
実態ファイル（Brewfile・plugins.lua・aliases.zsh 等）を読み込んで setup.md を自動更新し、CHANGES.md に変更ログを追記する。
セッション終了前に必ず実行すること。更新をスキップする場合は理由を明示する。

設定ファイルを更新した場合は、必要に応じて以下を実行してリンク先へ反映する。

```bash
bash bin/install.sh --link-only
```

### Brewfile を最新に更新する

現在インストール済みのパッケージを Brewfile に反映：

```bash
brew bundle dump --force --file=~/dev/dotfiles/homebrew/Brewfile
```

その後、不要なものを手動で削除してからコミットする。

---

## コミットメッセージの規約

形式: `<scope>: <変更内容>`

| scope      | 対象                          |
|------------|------------------------------|
| `zsh`      | zsh 関連ファイル全般           |
| `tmux`     | `.tmux.conf`                 |
| `ghostty`  | Ghostty config               |
| `git`      | `.gitconfig`                 |
| `brew`     | `Brewfile`                   |
| `install`  | `bin/install.sh`             |
| `docs`     | `README.md` / `AGENTS.md` / `setup.md` / `CHANGES.md` |
| `codex`    | Codex hook / Codex app 設定   |

例：
```
zsh: fzf の補完設定を .zshrc に追加
tmux: ペイン移動キーバインドを hjkl に変更
ghostty: フォントサイズを 14 → 15 に変更
```

---

## 環境情報

- **OS**: macOS（Apple Silicon / Intel 両対応）
- **シェル**: zsh（Homebrew 版）
- **ターミナル**: Ghostty
- **マルチプレクサ**: tmux（prefix: `Ctrl-z`）
- **AI エージェント**: Codex CLI / Codex app（Claude Code は legacy）
- **プロンプト**: Oh My Posh（カスタムテーマ: `zsh/.zsh/ohmyposh-theme.json`）
- **パッケージ管理**: Homebrew
- **バージョン管理**: mise（Node.js / Python 等）

---

## トラブルシューティングの定石

| 症状 | 確認箇所 |
|------|---------|
| zsh の設定が反映されない | `source ~/.zshrc` / シェルを再起動 |
| Homebrew コマンドが見つからない | `.zprofile` の `brew shellenv` が通っているか確認 |
| tmux 内で色がおかしい | `.tmux.conf` の `terminal-overrides` と `default-terminal` を確認 |
| Ghostty で日本語入力がおかしい | `font-feature` の設定と Ghostty のバージョンを確認 |
| `install.sh` でリンクが失敗する | `bash bin/install.sh --check` で状態確認 |

過去に発生した詳細な問題と解決策は [`docs/troubleshooting.md`](docs/troubleshooting.md) を参照。

<!-- BEGIN CODEX-HQ MANAGED RULES -->
## codex-hq 共通ルール

- ドキュメント、メモ、プレイブック、コメントは、基本的に日本語で書いてください。
- コマンド名、ファイル名、コード、設定キー、外部仕様名などは、必要に応じて英語のまま扱ってください。
- 現在のタスク状態は、プロジェクトルートの `TASK.md` を正として確認・更新してください。
- `TASK.md` は `codex-hq` から状態確認される前提で、現在地が分かる粒度で記録してください。
- このプロジェクトで利用するスキル、パス、更新日、プロジェクト固有かどうか、同期方針は `SKILLS.md` に記録してください。
- プロジェクト固有ではないスキルは、`codex-hq` から同期できる前提で扱ってください。
- プロジェクト固有のスキルを作る場合は、ユーザーから明示がない限りこのプロジェクト内の `.agents/skills/` 配下に作成してください。
- 既存の変更を勝手に戻さず、依頼対象の変更だけを扱ってください。
- ルール化済みの作業で追加指示や問題が出た場合は、対応と同時に該当するルールやスキル自身も更新してください。
<!-- END CODEX-HQ MANAGED RULES -->
